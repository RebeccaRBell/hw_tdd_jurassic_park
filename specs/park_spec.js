const assert = require('assert');
const Park = require('../models/park.js');
const Dinosaur = require('../models/dinosaur.js');

describe('Park', function() {
   let trex1;
  let trex2;
  let trex3;
  let velociraptor1;
  let velociraptor2;
  let diplodocus;
  let gallimimus;
  let park;
  beforeEach(function () {
      trex1 = new Dinosaur('t-rex', 'carnivore', 50);
    trex2 = new Dinosaur('t-rex', 'carnivore', 40);
    trex3 = new Dinosaur('t-rex', 'carnivore', 60);

    velociraptor1 = new Dinosaur('velociraptor', 'carnivore', 25);
    velociraptor2 = new Dinosaur('velociraptor', 'carnivore', 20);

    diplodocus = new Dinosaur('diplodocus', 'herbivore', 30);
    gallimimus = new Dinosaur('gallimimus', 'omnivore', 4);

    park = new Park('Jurassic Park', 100, [trex1, velociraptor2, gallimimus]);
    })

  it('should have a name', function(){
    const actual = park.name;
    assert.strictEqual(actual, 'Jurassic Park');
  });

  it('should have a ticket price', function(){
    const actual = park.ticket_price;
    assert.deepStrictEqual(actual, 100);
  });

  it('should have a collection of dinosaurs', function(){
    const actual = park.dinosaurs;
    assert.deepStrictEqual(actual, []);
  });

  it('should be able to add a dinosaur to its collection', function(){
    park.addDinosaur(diplodocus);
    assert.strictEqual(park.dinosaurCount(), 1);

  });

  it('should be able to remove a dinosaur from its collection', function(){
    park.addDinosaur(diplodocus);
    park.addDinosaur(velociraptor1);
    park.removeDinosaur();
    assert.deepStrictEqual(park.dinosaurs, [velociraptor1]);
  });

  it('should be able to find the dinosaur that attracts the most visitors', function(){
    park.addDinosaur(velociraptor1);
    park.addDinosaur(velociraptor2);
    park.addDinosaur(trex1);
    park.addDinosaur(trex2);
    park.addDinosaur(trex3);
    park.dinoWithMostVisitors();
    assert.strictEqual(park.dinoWithMostVisitors(), trex3)
  });

  it('should be able to find all dinosaurs of a particular species', function(){
    park.addDinosaur(velociraptor1);
    park.addDinosaur(velociraptor2);
    park.addDinosaur(trex1);
    park.addDinosaur(trex2);
    park.addDinosaur(trex3);
    park.findAllOfSpecies('t-rex');
    assert.deepStrictEqual(park.findAllOfSpecies('t-rex'), [trex1, trex2, trex3]);
  });

  it('should be able to calculate the total number of visitors per day', function(){
    park.addDinosaur(diplodocus);
    park.addDinosaur(gallimimus);
    park.addDinosaur(velociraptor1);
    park.addDinosaur(velociraptor2);
    park.addDinosaur(trex1);
    park.addDinosaur(trex2);
    park.addDinosaur(trex3);
    park.totalVisitorsPerDay();
    assert.strictEqual(park.totalVisitorsPerDay(), 229);
  });

  it('should be able to calculate the total number of visitors per year', function(){
    park.addDinosaur(diplodocus);
    park.addDinosaur(gallimimus);
    park.addDinosaur(velociraptor1);
    park.addDinosaur(velociraptor2);
    park.addDinosaur(trex1);
    park.addDinosaur(trex2);
    park.addDinosaur(trex3);
    park.totalVisitorsPerDay();
    park.totalVisitorsPerYear();
    assert.strictEqual(park.totalVisitorsPerYear(), 83585);
    
  });

  it('should be able to calculate total revenue for one year', function(){
    park.addDinosaur(diplodocus);
    park.addDinosaur(gallimimus);
    park.addDinosaur(velociraptor1);
    park.addDinosaur(velociraptor2);
    park.addDinosaur(trex1);
    park.addDinosaur(trex2);
    park.addDinosaur(trex3);
    park.totalVisitorsPerYear();
    assert.deepStrictEqual(park.annualRevenue(), 8358500)
  });

});
